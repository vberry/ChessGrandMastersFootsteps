...
class ChessGameNormal:
    ...
    def save_board_fen(self):
        """Sauvegarde la position actuelle de l'échiquier en notation FEN."""
        with open("fichierFenAjour.fen", "w") as fen_file:
    ...
    def process_next_position(self, current_comment="", opponent_comment=""):
        """Avance à la position suivante après un coup correct ou quand les tentatives sont épuisées"""
        ...
        self.remaining_attempts = 3  # Réinitialiser les tentatives
    ...
    def submit_move(self, move):
        ...
        counter = 0
        while not is_valid and counter < 3:
            is_valid, validated_move, error_message = self.validate_input(
                convertir_notation_francais_en_anglais(move.strip()).lower()
            )
            counter += 1
